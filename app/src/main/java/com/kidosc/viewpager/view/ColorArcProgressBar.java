...
public class ColorArcProgressBar extends View {
    ...
    private float startAngle = 125;//开始角度(0°与控件X轴平行)
    private float sweepAngle = 290;//弧形扫过的区域
    ...
    private int hintColor = 0xFFFFFFFF;
    ...
    private void initConfig(Context context, AttributeSet attrs) {
        ...
        colors = getResources().getIntArray(R.array.progressbarColor);
    ...
    protected void onSizeChanged(int w, int h, int oldw, int oldh) {
        ...
        Log.v("xulinchao", "onSizeChanged: diameter:" + diameter + " longDegree = " + longDegree + " DEGREE_PROGRESS_DISTANCE = " +
                DEGREE_PROGRESS_DISTANCE + " progressWidth = " + progressWidth);
        ...
        bgRect.right = 300;
        bgRect.bottom = 330;
        Log.e("xulinchao", "onSizeChanged: top = " + bgRect.top + " left = " + bgRect.left + " right = " + bgRect.right + " bottom = "
                + bgRect.bottom);


        //圆心
        centerX = (bgRect.right + bgRect.left) / 2;
        centerY = (bgRect.top + bgRect.bottom) / 2;
        Log.v("xulinchao", "initView: " + diameter + " centerX = " + centerX + " centerY = " + centerY);
    ...
    private void initView() {
        ...
        vTextPaint.setColor(getResources().getColor(R.color.current_step_color, null));
        ...
        curSpeedPaint.setColor(getResources().getColor(R.color.title_color,null));
    ...
    protected void onDraw(Canvas canvas) {
        ...
        //整个弧,startAngle:开始角度（以时钟3点的方向为0°，顺时针为正方向） sweepAngle: 扫过角度（以时钟3点的方向为0°，顺时针为正方向）
        ...
        if (isNeedContent) {
            ...
            canvas.drawText(String.format("%.0f", currentValues), centerX, centerY + textSize * 0.6f, vTextPaint);
        }
        if (isNeedUnit) {
            mStepTextLength = vTextPaint.measureText(String.format("%.0f", currentValues));
            canvas.drawText(hintString, centerX + mStepTextLength / 2 + 15, centerY + textSize * 0.6f - 10, hintPaint);
        }
        if (isNeedTitle) {
            canvas.drawText(titleString, centerX, centerY + textSize + 10, curSpeedPaint);
        }
        invalidate();
...
//
        ...
        if (touchRadius > mTouchInvalidateRadius && (angle >= 0 && angle <= 300))//其实角度小于270就够了,但是弧度换成角度是不精确的,所以需要适当放大范围,不然有时候滑动不到最大值
        {
            validate = true;
        }
